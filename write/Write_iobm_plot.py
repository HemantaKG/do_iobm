...
y = []
y1= []
directory_path= "/home/hemant/icts_hemanta_ws/IO_Banchmarking/src/storage/write_cache"#path to data-sets

fig = plt.figure(figsize=(6,4))
fig.suptitle('Write Performance on raiz2 with cache', fontsize=10, fontweight='bold')
...
def ReadDataPoints(directory_path, thread):
    ...
    y= []
    y1=[]
    fileName= directory_path+"/file_write_thread"+str(thread)+".txt"
    fop = open(fileName, 'r')
    lines = fop.readlines()
    fop.close()
    ...
    def conv_strore(s):
        if "kB/s" in line.split(' ')[13]:
            s.append(float(line.split(' ')[12])/ 1000000) # convert to GB/s
        elif "MB/s" in line.split(' ')[13]:
            s.append(float(line.split(' ')[12])/ 1000) # convert to GB/s
        else:
...
def show_Plot():
    ...
    plt.grid()
    plt.savefig('write_performance_raidz2_cache.png',dpi=200)
    #plt.show()
